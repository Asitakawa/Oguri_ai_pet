...
class OnigiriCatchGame(BaseGame):
    ...
    def _follow_pointer(self) -> None:
        try:
            ...
            return
        nx = mx - self.pet.pet_size[0] // 2
        nx = max(0, min(nx, self.pet.screen_w - self.pet.pet_size[0]))
        if abs(nx - self.pet.x) > 1:
            self.pet.x = nx
            self.pet._move()
