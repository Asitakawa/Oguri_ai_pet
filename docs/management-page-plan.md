...
> 状态：**已全部实现**（阶段 0–6 完成，另有双主题与聊天记忆卡片两次追加）· 更新：2026-09-11
> 本文档保留为设计依据与决策记录；实际用法见 [README](../README.md)，开发约定见 [development.md](development.md)。
...
**完成情况**：阶段 0–6 全部落地（对应 `git log` 里的 `feat(web): 阶段0` … `阶段6 打磨`）。之后又追加了两轮功能：`feat(web): 仪表盘实时终端`（日志 REST + SSE 增量推送）、`feat(web): 设置页独立「聊天记忆」卡片`，以及 `style(web): 双主题切换（深色工业/暖色玻璃）`。

## 9. 开放点 / 后续可议

- 技能 SKILL.md 是否支持网页编辑（**当前仍为只读**，详情页只展示原文）
- token 是否落盘以便重启后旧页面自动恢复（**当前仍不落盘**，重启后需重新从右键菜单打开）
- 对话输入条 / 查看状态是否保留在桌宠菜单（**已确认保留**，见 README 右键菜单）

### 实现后暴露出来的已知问题

以下不在原计划范围内，是复查时发现的，记在这里避免重复排查：

- `vite.config.ts` 未配 `server.proxy`，而 `client.ts` 用相对路径请求 → `npm run dev` 下 API 全挂，只能靠 `npm run build` + 桌宠托管验证
- `router.ts` 只在视图对象变化时调用 `unmount`，同一路由重复 `navigate` 会重挂而不卸载 → `Dashboard` 的 EventSource 与 store 订阅泄漏
- 仪表盘统计量只在 `render()` 时读 store，SSE 推送不会刷新已挂载的页面
- `Settings.mount` 里 `getSettings()` 失败会 `return`，导致六个卡片的监听器全部没绑定（表单变砖）
- 记忆轮数前端限 5–250，后端 `_apply_system` 钳到 10–500，两边不一致
- 401（token 失效）在各视图里统一显示为「未连接桌宠」，误导排查方向