...
# 桌宠窗口尺寸。
#
# 素材是 **365×585 竖长**（比例 0.62），而窗口原来是 180×180 的方框。
# `thumbnail` 只能缩不能拉伸，所以竖长的图在方框里被压到 **112×180** ——
# 只占 180 宽里的 112，剩下的地方是空白，桌宠显得又小又糊。
#
# 窗口改成 (180, 288)（= 180 / 0.62）后角色能铺满 **180×288**：渲染像素量
# 提升到 2.6 倍，细节清楚得多，桌宠在屏幕上的大小也才是正常桌宠该有的样子。
#
# 注意：缩放系数本身没变（都是约 0.49，因为素材两个方向都比窗口大），
# 变化的是"角色最终占多少像素"。
#
# 若以后换素材，把这里的宽高比跟着素材调即可。
DEFAULT_PET_SIZE = (180, 288)
MIN_SCALE = 0.5
MAX_SCALE = 2.0
DEFAULT_SCALE = 1.0
